...
def generate_articles_via_gemini(raw_data, existing_titles=[]):
...
5. 【重要・重複防止】既に存在する以下の記事タイトルと、同一または類似するトピック（同じ事件、事故、提携、同じ日時のニュース）については、絶対に新規の要約記事を生成しないでください。まだ掲載されていない新しい情報・トピックのみをニュース記事に仕立ててください。
...
def main():
    ...
    existing_titles_list = [art.get("title", "") for art in existing_data]
    
    new_articles = generate_articles_via_gemini(all_raw, existing_titles_list)
    if not new_articles:
